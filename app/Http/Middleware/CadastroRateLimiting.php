...
class CadastroRateLimiting
{
    ...
    {
        // APENAS PARA REQUISIÇÕES POST DE CADASTRO
        if ($request->isMethod('POST') && $request->routeIs('cadastro.attempt')) {
    ...
    protected function checkRateLimit(Request $request): void
    {
        ...
        if (RateLimiter::tooManyAttempts($key, $maxAttempts)) {
            ...
            // LOG DE BLOQUEIO
            Log::channel('security')->warning('IP bloqueado por rate limiting no cadastro', [
                'ip' => $request->ip(),
            ...
            throw ValidationException::withMessages([
                'EMAIL' => "🚨 Muitas tentativas de cadastro. Aguarde {$seconds} segundos para tentar novamente.",
            ]);
        }
    }
    ...
    protected function shouldIncrementRateLimit(Response $response): bool
    {
        ...
        return $response->getStatusCode() === 302 && 
               str_contains($response->headers->get('location', ''), 'cadastro');
    }
